...
class InferenceHelper:
    ...
    def predict_pil(self, pil_image, visualized=False):
        ...
        img = self.toTensor(img).unsqueeze(0).float()
        bin_centers, pred = self.predict(img)
        print("\nCentres : {}".format(bin_centers.shape))
        # new_pred = pred[0].reshape((300, 768, 1))
        # print("\nFinal : {}".format(new_pred[480:520, 890:920]))
...
if __name__ == '__main__':
    ...
    for filename in glob("fog_estimation/val/*"):
        elems = filename.split("/")
        outputfile = "/".join(elems[:-2]) + "/depth_adabins/" + elems[-1]
        img = Image.open(filename)
        print("Input size : ", img.size)
        start = time()
        centers, pred = inferHelper.predict_pil(img)
        print(f"took :{time() - start}s")
        cv2.imwrite(outputfile, pred[0][0])
        print("----------------------------------------------------\n")